...
export default {
  ...
  computed: {
    //mapGetter里面是传递数组，因为getters计算没有划分模块
  ...
  methods: {
    ...
    changeOrder(flag) {
      //形参判断用户是点击综合还是价格
      //这里获取最开始的状态1
      let startFlag = this.searchParams.order.split(':')[0];
      let startSort = this.searchParams.order.split(':')[1];
      //准备一个新的order属性
      let newOrder = '';
      //点击的是综合
      if (flag === startFlag) {
        newOrder = `${flag}:${startSort == 'desc' ? 'asc' : 'desc'}`;
      }
      //点击的是价格
      else {
        newOrder = `${flag}:${'desc'}`;
